...
public class WordyServer extends wordyPOA {
    ...
    public String getWinner() {
        ...
        // Check for tie condition (both clients sent the same length of word)
        ...
        if (isTie) {
            return "TIE: Both clients sent words of the same length. Starting another round...";
        }

        // Find the winner based on the win counts
        for (Map.Entry<String, Integer> entry : clientWinCount.entrySet()) {
            String playerName = entry.getKey();
            int winCount = entry.getValue();

            if (winCount > currentWinCount) {
        ...
        for (Map.Entry<String, String> entry : clientWords.entrySet()) {
            ...
            if (word.equals(longestWord)) {
                clientWinCount.put(playerName, clientWinCount.getOrDefault(playerName, 0) + 1);
                return playerName + " won with word: " + longestWord;
            }
        }
